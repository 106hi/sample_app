...
describe '投稿画面のテスト' do
  ...
  context '投稿処理のテスト' do
    it '投稿のリダイレクト先は正しいか' do
      # fill_inの記述は以下２つのどちらかで代用できないか
      # expect(FactoryBot.build(:list)).to be_valid
      # expect(FactoryBot.build(:list)).to click_button
      fill_in 'list[title]', with: Faker::Lorem.characters(number:10)
      fill_in 'list[body]', eith: Faker::Lorem.characters(number:30)
      click_button '投稿'
...
describe '編集画面のテスト' do
  ...
  context '更新処理に関するテスト' do
    it '更新後のリダイレクト先は正しいか' do
      ...
      # expect(FactoryBot.build(:list)).to be_valid
      # expect(FactoryBot.build(:list)).to click_button
      fill_in 'list[title]', with: Faker::Lorem.charactors(number:10)
      fill_in 'list[body]', with: Faker::Lorem.charactors(number:30)
      click_button '保存'
